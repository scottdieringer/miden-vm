...
fn u32clz() {
    ...
    // bit representation of the 14703 is 00000000000000000011100101101111
    build_op_test!(asm_op, &[14703]).expect_stack(&[18]);
...
fn u32cto() {
    ...
    build_op_test!(asm_op, &[0]).expect_stack(&[0]);
    build_op_test!(asm_op, &[1]).expect_stack(&[1]);
    // bit representation of the 4185032763 is 11111001011100101000100000111011
    build_op_test!(asm_op, &[4185032763]).expect_stack(&[2]);
    build_op_test!(asm_op, &[4294967295]).expect_stack(&[32]);
}
